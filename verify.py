...
def validate_inputs():
    ...
    if len(errors) != 0:
        gh_action.error(f'Missing required inputs: {", ".join(errors)}, please see documentation for correct usage.')
...
def teamserver_headers():
    ...
    return {'Accept': 'application/json', 'Content-Type': 'application/json',
            'Api-Key': CONFIG['CONTRAST_API_KEY'], 'Authorization': CONFIG['CONTRAST_AUTHORIZATION']
            }

def get_request(path, parameters={}):
    """Send a GET request to TeamServer."""
    gh_action.debug(f'GET {path} {parameters}')
    response = requests.get(CONFIG['BASE_URL']+path, params=parameters, headers=teamserver_headers())
...
def post_request(path, body):
    ...
    response = requests.post(CONFIG['BASE_URL']+path, json=body, headers=teamserver_headers())
    response.raise_for_status()
    return response
...
def determine_application_id():
    ...
    matching_named_apps = list(filter(lambda app: app['name'] == CONFIG['APP_NAME'], applications))
    if len(matching_named_apps) != 1:
        gh_action.set_failed(f'Could not match one app with name "{CONFIG["APP_NAME"]}", found {len(matching_named_apps)}, consider using APP_ID input instead.')
    else:
...
def perform_security_check():
    ...
    response = post_request('securityChecks', {
...
def fetch_vulnerability_count():
    ...
    response = get_request(f'traces/{CONFIG["APP_ID"]}/quick', {
...
if security_check_result == False:
    ...
    gh_action.set_failed(f'Contrast verify gate fails with status {jop_outcome} - policy "{jop_name}"')
    ...
    gh_action.info('No matching job outcome policy, checking vulnerabilities against threshold...')
    response = fetch_vulnerability_count()
    gh_action.debug(response)
    open_vulnerabilities_data = next(filter(lambda filter: filter['filterType'] == 'OPEN', response['filters']))
    ...
    if open_vulnerabilities > CONFIG['FAIL_THRESHOLD']:
        ...
        gh_action.set_failed(f'Contrast verify gate fails as this is above threshold (threshold allows {CONFIG["FAIL_THRESHOLD"]})')
    else:
        gh_action.info(f'The vulnerability count is {open_vulnerabilities} (below threshold)')
